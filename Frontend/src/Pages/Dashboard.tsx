...
const Dashboard = () => {
  ...
  const getApplications = async () => {
    try {
      ...
      let url: string = `http://localhost:9080/api/application/get?shelterName=${shelterName}`;
  ...
  const updateApplicationStatus = async (
    ...
    let request: AppManageDto = {
      appId: applicationID,
      status: newStatus,
    };
    try {
      // TODO add the authorization header
      let url: string = `http://localhost:9080/api/application/manage`;
      ...
      // Handle errors here
      return "The application status can't be changed anymore";
    }
  };
